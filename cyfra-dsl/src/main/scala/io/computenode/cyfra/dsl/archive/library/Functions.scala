package io.computenode.cyfra.dsl.archive.library
...
import io.computenode.cyfra.dsl.archive.algebra.VectorAlgebra.{*, given}
...
object Functions:
  ...
  def length[T <: Scalar: Tag](v: Vec3[T])(using Source): Float32 = Float32(ExtFunctionCall(Len3, List(v)))
  ...
  def clamp(f: Float32, from: Float32, to: Float32)(using Source): Float32 =
  ...
  def logn(f: Float32)(using Source): Float32 = Float32(ExtFunctionCall(Log, List(f)))
  def log(f: Float32, base: Float32)(using Source): Float32 = logn(f) / logn(base)
