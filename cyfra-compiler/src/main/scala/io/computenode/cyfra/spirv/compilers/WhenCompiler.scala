...
private[cyfra] object WhenCompiler:
  ...
  def compileWhen(when: WhenExpr[?], ctx: Context): (List[Words], Context) = {
    def compileCases(ctx: Context, resultVar: Int, conditions: List[E[?]], thenCodes: List[E[?]], elseCode: E[?]): (List[Words], Context) =
      (conditions, thenCodes) match {
        ...
        case (caseWhen :: cTail, tCode :: tTail) =>
          val (whenInstructions, whenCtx) = compileBlock(caseWhen, ctx)
          val (thenInstructions, thenCtx) = compileBlock(tCode, whenCtx)
          val thenWithStore = thenInstructions :+ Instruction(Op.OpStore, List(ResultRef(resultVar), ResultRef(thenCtx.exprRefs(tCode.treeid))))
          ...
          val (elseInstructions, elseCtx) = compileCases(contextForNextIter, resultVar, cTail, tTail, elseCode)
          (
            whenInstructions ::: List(
              Instruction(Op.OpSelectionMerge, List(ResultRef(endIfLabel), SelectionControlMask.MaskNone)),
              Instruction(Op.OpBranchConditional, List(ResultRef(postCtx.exprRefs(caseWhen.treeid)), ResultRef(thenLabel), ResultRef(elseLabel))),
    ...
    val (caseInstructions, caseCtx) = compileCases(contextForCases, resultVar, when.exprDependencies.head :: conds, thenCode :: thenCodes, elseCode)
    val instructions =
