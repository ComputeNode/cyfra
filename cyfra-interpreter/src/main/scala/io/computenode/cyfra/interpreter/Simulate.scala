...
object Simulate:
  ...
  def simGroup(invocIds: List[Int], e: Expression[?]): List[InvocResult] =
    val topoSortedExpressions = buildBlock(e)
    invocIds.map: id =>
      val (res, sc) = simIterate(topoSortedExpressions, SimContext())(using Map())
      InvocResult(id, sc)
