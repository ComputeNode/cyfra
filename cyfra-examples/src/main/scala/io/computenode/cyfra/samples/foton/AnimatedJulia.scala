...
//import io.computenode.cyfra.dsl.archive.library.Color.{InterpolationThemes, interpolate}
...
//      val const = (p, p)
//      GSeq.gen(uv, next = v => ((v.x * v.x) - (v.y * v.y), 2.0f * v.x * v.y) + const).limit(1000).map(length).takeWhile(_ < 2.0f).count
//
