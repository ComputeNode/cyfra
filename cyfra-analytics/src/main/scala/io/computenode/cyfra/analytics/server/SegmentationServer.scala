  ...
  def run: IO[Unit] =
    resources.use { case (service, endpoints, dataGen) =>
      ...
      val swaggerRoutes = Http4sServerInterpreter[IO]().toRoutes(
        SwaggerInterpreter().fromEndpoints[IO](endpoints.all, "GPU Customer Segmentation API", "1.0.0")
      )
      ...
      for
        ...
        _ <- serverResource.use { _ =>
          IO(logger.info(s"Server: http://localhost:8081")) >>
          IO(logger.info(s"Swagger: http://localhost:8081/docs")) >>
          IO.never
        }
      yield ()
    }
