...
class DataGenerationService(
  ...
  private def generateTransactions(
    customerId: Int,
    count: Int,
    now: Long,
    day: Long,
    ...
    (1 to count).toList.map { _ =>
      ...
      Transaction(
        ...
        discountPct = if category == 4 then random.nextDouble() * 0.4 else if category == 0 then random.nextDouble() * 0.05 else random.nextDouble() * 0.15
      )
    }
