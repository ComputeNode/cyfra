...
class SegmentationService(
  ...
  /** Main processing pipeline: transactions → profiles → GPU segmentation → storage
    *
    * The GPU pipe (GpuAnalyticsPipeline.segment) sits directly in the stream.
    * Profiles flow as chunks through the GPU for parallel membership computation.
    */
  ...
  private def persistResults: Pipe[IO, Chunk[(CustomerProfile, CustomerSegment)], Unit] =
    _.evalMap { chunk =>
      ...
      profileRepo.upsertBatch(profiles) *>
        segmentRepo.upsertBatch(segments) *>
        lastUpdateRef.set(System.currentTimeMillis())
    }
...
object SegmentationService:
  def create(
    ...
    yield new SegmentationService(
      profileRepo, segmentRepo, centroidsRepo, runtime,
      queue, lastUpdate, txCache, centroidsCache
    )
