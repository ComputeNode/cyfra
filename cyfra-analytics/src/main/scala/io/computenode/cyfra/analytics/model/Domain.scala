...
case class Transaction(customerId: Long, timestamp: Long, amount: Double, items: Int, category: Int, channel: String, discountPct: Double)
...
case class CustomerSegment(customerId: Long, segmentName: String, confidence: Float, memberships: Map[String, Float], assignedAt: Long)
...
/** Segment statistics */
case class SegmentInfo(name: String, customerCount: Int, avgLifetimeValue: Double, avgRecency: Double)
...
case class SegmentsResponse(segments: List[SegmentInfo], totalCustomers: Int, lastUpdated: Long)
