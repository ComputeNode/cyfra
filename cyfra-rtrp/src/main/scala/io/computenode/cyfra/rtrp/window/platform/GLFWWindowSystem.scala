...
class GLFWWindowSystem extends WindowSystem {
  ...
  override def initialize(): Try[Unit] = Try {
    if initialized.get() then throw new IllegalStateException("WindowSystem is already initialized")
    ...
    if !glfwVulkanSupported() then throw new RuntimeException("GLFW: Vulkan is not supported on this system")
  ...
  override def createWindow(config: WindowConfig): Try[Window] = Try {
    if !initialized.get() then throw new IllegalStateException("WindowSystem not initialized")
    ...
    if windowPtr == NULL then throw new RuntimeException("Failed to create GLFW window")
