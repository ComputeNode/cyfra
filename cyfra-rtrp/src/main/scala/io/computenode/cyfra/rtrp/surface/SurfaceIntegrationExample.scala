...
import io.computenode.cyfra.rtrp.surface.core.{SurfaceConfig, PresentMode}
...
object SurfaceIntegrationExample {
  ...
  private def createTestWindows(manager: WindowManager): List[(io.computenode.cyfra.rtrp.window.core.Window, io.computenode.cyfra.rtrp.surface.core.RenderSurface)] = {
    val configs = List(
      ...
      (WindowConfig(
        ...
        position = Some(WindowPosition.Fixed(100, 100))
      ), SurfaceConfig.quality),
      ...
      (WindowConfig(
        ...
        height = 300,
        title = "Tool Window",
        position = Some(WindowPosition.Fixed(200, 200))
      ), SurfaceConfig.lowLatency)
    )
  ...
  private def inspectSurfaceCapabilities(windowSurfacePairs: List[(io.computenode.cyfra.rtrp.window.core.Window, io.computenode.cyfra.rtrp.surface.core.RenderSurface)]): Unit = {
    windowSurfacePairs.foreach { case (window, surface) =>
      ...
      surface.getCapabilities() match {
        case Success(caps) =>
          ...
          println(s"Formats (${caps.supportedFormats.size}): ${caps.supportedFormats.take(3).mkString(", ")}${if (caps.supportedFormats.size > 3) "..." else ""}")
  ...
  private def runMainLoop(
    ...
    while (frameCount < maxFrames && windows.exists(!_.shouldClose)) {
      ...
      if (frameCount % 60 == 0) {
        ...
        manager.getSurfaceManager().foreach { surfMgr =>
