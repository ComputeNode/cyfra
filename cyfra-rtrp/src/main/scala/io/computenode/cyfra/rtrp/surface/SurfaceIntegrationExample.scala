...
object SurfaceIntegrationExample:
  ...
  def main(args: Array[String]): Unit =
    logger.info("=== Cyfra Surface Integration Example ===")

    val result = runFullExample()

    result match
      case Success(_) =>
        logger.info("Surface integration example completed successfully!")
      case Failure(ex) =>
        logger.error(s"Surface integration example failed: ${ex.getMessage}")
        ex.printStackTrace()
    ...
    try
      ...
      WindowManager.withVulkanManager(vulkanContext): manager =>
        Try:
          ...
          logger.info(s"Created ${windowsAndSurfaces.size} window-surface pairs\n")
  ...
  private def setupEventHandlers(manager: WindowManager): Unit =
    ...
    manager.onWindowResize: event =>
      logger.info(s"Window ${event.windowId} resized to ${event.width}x${event.height}")

    manager.onWindowClose: event =>
      logger.info(s"Window ${event.windowId} close requested")

    manager.onKeyPress: event =>
      logger.info(s"Key ${event.key.code} pressed in window ${event.windowId}")

    manager.onMouseClick: event =>
      logger.info(s"Mouse button ${event.button.code} clicked at (${event.x.toInt}, ${event.y.toInt}) in window ${event.windowId}")

    // Surface event handlers
    manager.onSurfaceCreated: event =>
      logger.info(s"Surface ${event.surfaceId} created for window ${event.windowId}")
      val caps = event.capabilities
      logger.info(s"Formats: ${caps.supportedFormats.size}, Present modes: ${caps.supportedPresentModes.size}")

    manager.onSurfaceDestroyed: event =>
      logger.info(s"Surface ${event.surfaceId} destroyed for window ${event.windowId}")

    manager.onSurfaceLost: event =>
      logger.warn(s"Surface ${event.surfaceId} lost for window ${event.windowId}: ${event.error}")
  ...
  private def createTestWindows(
    ...
    manager.createWindowsWithSurfaces(configs) match
      ...
      case Failure(ex)    =>
        logger.error(s"Failed to create windows: ${ex.getMessage}")
        List.empty
  ...
  private def inspectSurfaceCapabilities(
    ...
    windowSurfacePairs.foreach { case (window, surface) =>
      ...
      surface.getCapabilities() match
        case Success(caps) =>
          logger.info(s"Current size: ${caps.currentExtent}")
          logger.info(s"Size range: ${caps.minImageExtent} to ${caps.maxImageExtent}")
          logger.info(s"Image count: ${caps.minImageCount} to ${caps.maxImageCount}")
          logger.info(
            s"Formats (${caps.supportedFormats.size}): ${caps.supportedFormats.take(3).mkString(", ")}${if caps.supportedFormats.size > 3 then "..." else ""}",
          )
          logger.info(s"Present modes (${caps.supportedPresentModes.size}): ${caps.supportedPresentModes.mkString(", ")}")
          logger.info(s"Alpha support: ${caps.supportsAlpha}")
          logger.info(s"Transform support: ${caps.supportsTransform}")

        case Failure(ex) =>
          logger.error(s"Failed to get capabilities: ${ex.getMessage}")
    }
  ...
  private def runMainLoop(
    ...
    while frameCount < maxFrames && windows.exists(!_.shouldClose) do
      ...
      manager.pollAndDispatchEvents() match
        ...
        case Failure(ex) => logger.warn(s"Event polling failed: ${ex.getMessage}")
      ...
      if frameCount % 60 == 0 then
        ...
        logger.info(s"${seconds}s: $openWindows windows open, $validSurfaces surfaces valid")

        // Log surface manager statistics
        manager
          .getSurfaceManager()
          .foreach: surfMgr =>
            val stats = surfMgr.getStatistics()
            logger.info(s"Surface stats: $stats")
    ...
    manager.getSurfaceManager() match
      case Some(surfMgr) =>
        surfMgr.recreateSurface(surface.windowId, "Test recreation") match
          case Success(_) =>
            logger.info("Surface recreation successful")
          case Failure(ex) =>
            logger.error(s"Surface recreation failed: ${ex.getMessage}")
      case None =>
        logger.warn("No surface manager available")
