...
class VulkanSurface(
  ...
  override def destroy(): Try[Unit] = Try {
    if (!destroyed.getAndSet(true)) {
      try {
        vkDestroySurfaceKHR(vulkanContext.instance.get, nativeHandle, null)
      } finally {
        surfaceCapabilities = None
        lastKnownSize = None
      }
...
// Check if this surface supports presentation on the given queue family
  def supportsPresentationOnQueueFamily(queueFamilyIndex: Int): Try[Boolean] = Try {
    ...
    try {
      ...
      if (result != VK_SUCCESS) {
        throw new RuntimeException(s"Failed to check surface support: $result")
      }
  ...
  private def checkValid(): Unit = {
    if (!isValid) {
      throw new IllegalStateException("Surface is not valid or has been destroyed")
    }
  }
}