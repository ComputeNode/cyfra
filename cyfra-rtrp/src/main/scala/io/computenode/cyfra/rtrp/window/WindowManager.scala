...
class WindowManager {
  ...
  def createWindowsWithSurfaces(
    configs: List[(WindowConfig, SurfaceConfig)]
  ): Try[List[(Window, RenderSurface)]] = {
  ...
  def shutdown(): Try[Unit] = {
    val results = List(
      surfaceManager.map(_.shutdown()).getOrElse(Success(())),
      windowSystem.map(_.shutdown()).getOrElse(Success(()))
    )
