...
class VulkanSurfaceFactory(vulkanContext: VulkanContext) {
  ...
  def createSurface(window: Window, config: SurfaceConfig): Try[VulkanSurface] = Try {
    if (window.nativeHandle == 0L) {
      throw new IllegalArgumentException("Window native handle is null")
    }
    if (vulkanContext.instance.get.address() == 0L) {
      throw new IllegalStateException("VulkanContext instance is null")
    }
  ...
  private def createVulkanSurface(windowHandle: Long): Long = {
    ...
    try {
      ...
      val result = GLFWVulkan.glfwCreateWindowSurface(
      ...
      if (result != VK_SUCCESS) {
        throw new RuntimeException(s"Failed to create Vulkan surface: $result")
      }
      
      val surfaceHandle = pSurface.get(0)
      
      if (surfaceHandle == 0L) {
        throw new RuntimeException("Created surface handle is null")
      }
  ...
  private def validateSurfaceConfig(
    surface: VulkanSurface, 
    config: SurfaceConfig, 
    capabilities: SurfaceCapabilities
  ): Unit = {
    if (!capabilities.supportsFormat(config.preferredFormat)) {
      println(s"Warning: Preferred format ${config.preferredFormat} not supported by surface ${surface.id}")
    }
    
    if (!capabilities.supportsPresentMode(config.preferredPresentMode)) {
    ...
    config.minImageCount.foreach { minCount =>
      if (minCount < capabilities.minImageCount) {
    ...
    config.maxImageCount.foreach { maxCount =>
      if (maxCount > capabilities.maxImageCount && capabilities.maxImageCount != Int.MaxValue) {
