...
class rtrpExample:
  ...
  private def createIndexBuffer(): Unit =
    ...
    for index <- indices do data.putShort(index)
    data.rewind()
  ...
  def drawFrame(): Unit = pushStack: stack =>
    ...
    val acquireResult =
      vkAcquireNextImageKHR(device.get, swapchain.get, Long.MaxValue, imageAvailableSemaphores(currentFrame).get, VK_NULL_HANDLE, pImageIndex)
    if acquireResult == VK_ERROR_OUT_OF_DATE_KHR then
      recreateSwapchain()
      return
    else if acquireResult != VK_SUCCESS && acquireResult != VK_SUBOPTIMAL_KHR then throw RuntimeException("failed to acquire swap chain image!")
    ...
    if presentResult == VK_ERROR_OUT_OF_DATE_KHR || presentResult == VK_SUBOPTIMAL_KHR then recreateSwapchain()
    else if presentResult != VK_SUCCESS then throw RuntimeException("failed to present swap chain image!");
  ...
  def cleanup(): Unit =
    ...
    catch case t: Throwable => println(s"[cleanup] error: ${t.getMessage}")
