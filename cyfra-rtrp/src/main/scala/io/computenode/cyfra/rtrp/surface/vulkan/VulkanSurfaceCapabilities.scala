...
class VulkanSurfaceCapabilities(vulkanContext: VulkanContext, surface: VulkanSurface) extends SurfaceCapabilities:
  ...
  private val (
    ...
    try
      ...
      val result = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vulkanContext.device.physicalDevice, surface.nativeHandle, caps)
      ...
      val current =
        if curExtent.width() == 0xffffffff || curExtent.height() == 0xffffffff then (-1, -1)
        else (curExtent.width(), curExtent.height())

      val supportsAlpha = (caps.supportedCompositeAlpha() & VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR) != 0
      val supportsTransform = (caps.supportedTransforms() & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR) != 0

      val minImageCount = caps.minImageCount()
      val maxImageCount =
        if caps.maxImageCount() == 0 then Int.MaxValue else caps.maxImageCount()

      (minExtent, maxExtent, current, minImageCount, maxImageCount, supportsAlpha, supportsTransform)
    finally MemoryStack.stackPop()

  // Query formats and present modes once and copy into safe lists of ints
  private val (formatsList, colorSpacesList, presentModesList) =
    MemoryStack.stackPush()
    try
      val stack = MemoryStack.stackGet()
      ...
      val formats =
        if formatCount == 0 then List.empty[(Int, Int)]
          ...
          vkGetPhysicalDeviceSurfaceFormatsKHR(vulkanContext.device.physicalDevice, surface.nativeHandle, pFormatCount, fmtBuf)
      ...
      val presentModes =
        if modeCount == 0 then List.empty[Int]
          ...
          vkGetPhysicalDeviceSurfacePresentModesKHR(vulkanContext.device.physicalDevice, surface.nativeHandle, pModeCount, modesBuf)
  ...
  override def vkSurfaceFormats: List[VkSurfaceFormatKHR] =
    val stack = MemoryStack.stackGet()
    if stack == null then throw new RuntimeException("vkSurfaceFormats must be called with an active MemoryStack")
    val pFormatCount = stack.callocInt(1)
    vkGetPhysicalDeviceSurfaceFormatsKHR(vulkanContext.device.physicalDevice, surface.nativeHandle, pFormatCount, null)
