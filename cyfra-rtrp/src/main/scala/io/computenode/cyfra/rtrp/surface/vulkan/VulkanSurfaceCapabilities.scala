...
class VulkanSurfaceCapabilities(
  ...
  override def maxImageCount: Int = {
    ...
    if (max == 0) Int.MaxValue else max // 0 means no limit
  }
  
  override def supportsAlpha: Boolean = {
    (vkCapabilities.supportedCompositeAlpha() & VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR) != 0
  }
  
  override def supportsTransform: Boolean = {
    (vkCapabilities.supportedTransforms() & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR) != 0
  }
  ...
  private def queryAvailableFormats(): List[VkSurfaceFormatKHR] = {
    ...
    try {
      ...
      if (formatCount == 0) return List.empty
  ...
  private def queryPresentModes(): List[Int] = {
    ...
    try {
      ...
      if (modeCount == 0) return List.empty
