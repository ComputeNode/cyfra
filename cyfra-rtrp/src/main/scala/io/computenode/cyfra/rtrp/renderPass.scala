...
private[cyfra] class RenderPass(context: VulkanContext, swapchain: Swapchain) extends VulkanObjectHandle:
  ...
  protected val handle: Long = pushStack: stack =>
    ...
    if vkCreateRenderPass(device.get, renderPassInfo, null, pRenderPass) != VK_SUCCESS then throw new RuntimeException("failed to create render pass!")
    pRenderPass.get(0)
  ...
  def recordCommandBuffer(
    ...
    try
      ...
      case t: Throwable =>
        if !finished then
          try vkEndCommandBuffer(commandBuffer)
          catch case _: Throwable => ()
        false

  def destroyFramebuffers(): Unit =
    if swapchainFramebuffers != null then for fb <- swapchainFramebuffers do if fb != 0L then vkDestroyFramebuffer(device.get, fb, null)
