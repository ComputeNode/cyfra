...
/** Writes K vectors to KV cache at specified positions (F32 version).
  *
  ...
  def forward(sizes: Sizes): GProgram[Sizes, ProgramLayout] =
    ...
    GProgram[Sizes, ProgramLayout](
      layout = s => ProgramLayout(
        k = GBuffer[Float32](s.B * s.T * s.NKV * s.headSize),
