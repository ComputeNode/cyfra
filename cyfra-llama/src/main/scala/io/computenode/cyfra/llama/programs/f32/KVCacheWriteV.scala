...
/** Writes V vectors to KV cache at specified positions (F32 version).
  *
  * Identical structure to KVCacheWriteK but operates on V vectors.
  */
object KVCacheWriteV:
  ...
  def forward(sizes: Sizes): GProgram[Sizes, ProgramLayout] =
    ...
    GProgram[Sizes, ProgramLayout](
      layout = s => ProgramLayout(
        v = GBuffer[Float32](s.B * s.T * s.NKV * s.headSize),
