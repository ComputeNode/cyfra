...
/** KV-cached attention for incremental inference (F16 version).
  *
  * Computes attention by reading Q from current tokens and K/V from the full cache.
  * Uses F32 for intermediate score computations for numerical stability, F16 for storage.
  *
  ...
  def forward(sizes: Sizes): GProgram[Sizes, ProgramLayout] =
    ...
    GProgram[Sizes, ProgramLayout](
      ...
      val computeScoresAndMax: GIO[Float32] = GIO.foldRepeat[Float32](numScoreIterations, -10000.0f): (iter, localMax) =>
        ...
        val dot = GSeq.gen[Int32](0, _ + 1).limit(headSize).unroll.fold(0.0f, (acc: Float32, d: Int32) =>
          val qVal = GIO.read[Float16](layout.q, qBase + d).asFloat32
          val kVal = GIO.read[Float16](layout.kCache, kCacheBase + d).asFloat32
          acc + qVal * kVal
        )
      ...
      for
        ...
        _ <- GIO.when(outDim1 < headSizeVal):
          ...
          GIO.write[Float16](layout.output, outBase + outDim1, weightedSum1.asFloat16)
        ...
        _ <- GIO.when(outDim2 < headSizeVal):
          ...
          val outBase = batchIdx * Tval * NHval * headSizeVal + queryPosLocal * NHval * headSizeVal + headIdx * headSizeVal
          GIO.write[Float16](layout.output, outBase + outDim2, weightedSum2.asFloat16)
      yield Empty()
