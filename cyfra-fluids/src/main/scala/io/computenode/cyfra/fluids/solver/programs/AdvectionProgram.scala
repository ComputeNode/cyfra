  ...
  def create: GProgram[Int, FluidStateDouble] =
    GProgram[Int, FluidStateDouble](
      ...
      GIO.when(idx < totalCells):
        ...
        val interpolatedVel = trilinearInterpolateVec4(state.velocityPrevious, prevPos, n)

        val interpolatedDensity = trilinearInterpolateFloat32(state.densityPrevious, prevPos, n)

        val interpolatedTemperature = trilinearInterpolateFloat32(state.temperaturePrevious, prevPos, n)

        val interpolatedDye = trilinearInterpolateFloat32(state.dyePrevious, prevPos, n)
