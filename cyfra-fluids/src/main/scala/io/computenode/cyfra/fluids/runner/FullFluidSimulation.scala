...
import io.computenode.cyfra.fluids.solver.utils.*
...
object FullFluidSimulation:
  ...
  def buildPipeline(renderDim: (Int, Int), jacobiIters: Int): GExecution[Int, SimulationLayout, SimulationLayout] =
    GExecution[Int, SimulationLayout]()
      ...
      .addProgram(ForcesProgram.create)(totalCells => totalCells, _.toFluidStatePrevious)
      // 1b. Vorticity Confinement - Add swirling motion to velocityPrevious
      .addProgram(VorticityConfinementProgram.create)(totalCells => totalCells, _.toFluidStatePrevious)
      // 2. Advection - Reads velocityPrevious WITH forces, writes velocityCurrent
      .addProgram(AdvectionProgram.create)(totalCells => totalCells, _.toFluidStateDouble)
      // 3. Diffusion - Apply viscosity to velocity
      .addProgram(DiffusionProgram.create)(totalCells => totalCells, _.toFluidStateDouble)
      // 4. Projection - Enforce incompressibility (∇·u = 0)
      // 4a. Compute divergence
      .addProgram(ProjectionProgram.divergence)(totalCells => totalCells, _.toFluidStateCurrent)
      // 4b. Solve Poisson equation for pressure (40 Jacobi iterations)
      .addProgram(ProjectionProgram.pressureSolve)(totalCells => totalCells, _.toFluidStateDouble)
      ...
      .addProgram(ProjectionProgram.subtractGradient)(totalCells => totalCells, _.toFluidStateCurrent)
      // 5. Boundary conditions - apply AFTER all simulation steps
      .addProgram(OutflowBoundaryProgram.create)(totalCells => totalCells, _.toFluidStateCurrent)
      // 6. Render
      .addProgram(
        RayMarchRenderer(
          RendererConfig(width = renderDim._1, height = renderDim._2, fieldToRender = Dye, renderOver = 0.0f, renderMax = 1.0f),
        ).renderProgram,
      )(
  ...
  @main def runFullFluidSimulation(): Unit =
    ...
    try
      ...
      val region = (0 until numFrames).foldLeft(
        GBufferRegion
        ...
        regionAcc.map: layout =>
          ...
          updateScene(velocityPrevious, dyePrevious, gridSize, frameIdx)
  ...
  /** Add dsc-shaped velocity vector.
    *
    * This should be called EVERY FRAME to continuously inject velocity. Creates two disc-shaped regions with strong velocities flowing toward each
    * other.
    *
    * @param velocityBuffer
    *   Velocity field to modify
    * @param dyeBuffer
    *   Dye field to modify
    * @param gridSize
    *   Grid resolution
    ...
    *   Current frame index
    */
  def updateScene(velocityBuffer: ByteBuffer, dyeBuffer: ByteBuffer, gridSize: Int, frameIdx: Int): Unit =
    val center = gridSize / 2.0f
    val currentRadius = gridSize * 0.15f
    val separation = gridSize * 0.35f
