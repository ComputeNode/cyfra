...
object FluidColorMap:
  ...
  /** Simple grayscale mapping based on density.
    * 
    * @param density Fluid density value (0.0 to 1.0)
    * @return RGBA color with density as grayscale
    */
  ...
  /** Temperature-based color ramp (cold blue to hot red).
    ...
    * - 1.0 = white
    * 
    * @param density Fluid density value
    * @return RGBA color on the heat map
    */
  def heatMap(density: Float32)(using Source): Vec4[Float32] =
    ...
    val g = when(d < 0.25f)(0.0f).otherwise(
      when(d < 0.75f)((d - 0.25f) * 2.0f).otherwise(1.0f)
    )
  ...
  /** Smoke-like appearance with soft falloff.
    ...
    * 
    * @param density Fluid density value
    * @return RGBA color with alpha for blending
    */
  ...
  /** Velocity magnitude-based coloring.
    ...
    * @param maxSpeed Maximum expected speed for normalization
    * @return RGBA color representing speed
    */
