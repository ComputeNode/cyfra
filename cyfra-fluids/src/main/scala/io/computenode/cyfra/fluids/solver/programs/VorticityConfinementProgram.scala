...
/** Vorticity Confinement Program
  *
  * Adds vorticity confinement forces to restore small-scale rolling features that are lost due to numerical dissipation.
  *
  * Based on: Fedkiw et al. "Visual Simulation of Smoke" (SIGGRAPH 2001) and GPU Gems Chapter 38: "Fast Fluid Dynamics Simulation on the GPU"
  *
  * The algorithm:
      ...
      GIO.when(idx < totalCells):
        ...
        GIO.when(isInterior):
          ...
          val N = when(gradMagLength > epsilon):
            vec3(gradMagX / gradMagLength, gradMagY / gradMagLength, gradMagZ / gradMagLength)
          .otherwise:
            vec3(0.0f, 0.0f, 0.0f)
          ...
          val confinementForce = vec3(forceX * vorticityEpsilon, forceY * vorticityEpsilon, forceZ * vorticityEpsilon)

          val vel = state.velocity.read(idx)
          val newVel =
            vec4(vel.x + confinementForce.x * params.dt, vel.y + confinementForce.y * params.dt, vel.z + confinementForce.z * params.dt, 0.0f)
