...
/** Camera parameters for 3D rendering.
  *
  * Uses Vec4 instead of Vec3 for proper 16-byte alignment in GPU memory. The w component is unused for position/target/up vectors.
  *
  * @param position
  *   Camera position in world space (w unused)
  * @param target
  *   Point the camera is looking at (w unused)
  * @param up
  *   Up vector (typically (0, 1, 0, 0)) (w unused)
  * @param fov
  *   Field of view in degrees
  * @param aspectRatio
  *   Width / height ratio
  */
case class Camera3D(position: Vec4[Float32], target: Vec4[Float32], up: Vec4[Float32], fov: Float32, aspectRatio: Float32) extends GStruct[Camera3D]
...
object Camera3D:
  ...
  def orbit(centerX: Float, centerY: Float, centerZ: Float, radius: Float, angle: Float, height: Float, aspectRatio: Float): Camera3D =
