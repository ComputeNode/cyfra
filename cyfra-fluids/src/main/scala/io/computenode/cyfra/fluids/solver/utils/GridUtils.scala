...
object GridUtils:
  ...
  /** Convert 3D coordinates to 1D flattened index. Uses row-major order: index = x + y*N + z*N²
    */
  ...
  def readVec4Safe(buffer: GBuffer[Vec4[Float32]], x: Int32, y: Int32, z: Int32, n: Int32)(using Source): Vec4[Float32] =
  ...
  def readFloat32Safe(buffer: GBuffer[Float32], x: Int32, y: Int32, z: Int32, n: Int32)(using Source): Float32 =
    val xClamped = maxInt32(0, minInt32(x, n - 1))
    val yClamped = maxInt32(0, minInt32(y, n - 1))
    val zClamped = maxInt32(0, minInt32(z, n - 1))
    buffer.read(coord3dToIdx(xClamped, yClamped, zClamped, n))

  /** Trilinear interpolation for Vec4 field. Samples 8 surrounding grid points and blends them. Note: pos only uses x,y,z components
    */
  def trilinearInterpolateVec4(buffer: GBuffer[Vec4[Float32]], pos: Vec3[Float32], size: Int32)(using Source): Vec4[Float32] =
  ...
  def trilinearInterpolateFloat32(buffer: GBuffer[Float32], pos: Vec3[Float32], size: Int32)(using Source): Float32 =
    val maxPos = (size - 1).asFloat
    val x = clamp(pos.x, 0.0f, maxPos)
    val y = clamp(pos.y, 0.0f, maxPos)
    val z = clamp(pos.z, 0.0f, maxPos)
