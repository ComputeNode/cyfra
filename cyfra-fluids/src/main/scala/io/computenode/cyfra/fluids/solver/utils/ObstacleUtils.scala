...
object ObstacleUtils:
  ...
  /** Check if a 3D coordinate is inside a solid obstacle with bounds checking. Returns false if coordinates are out of bounds.
    */
  ...
  /** Get obstacle color value (0.0-1.0, returns 0 if not solid). Note: Does not perform bounds checking.
    */
  ...
  /** Compute surface normal for obstacle visualization (GPU side).
    *
    * Uses central differences to estimate the gradient of the obstacle field. Returns normalized vector pointing away from solid.
    */
  def computeNormal(obstacles: GBuffer[Float32], x: Int32, y: Int32, z: Int32, n: Int32)(using
    io.computenode.cyfra.dsl.macros.Source,
  ): Vec3[Float32] =
