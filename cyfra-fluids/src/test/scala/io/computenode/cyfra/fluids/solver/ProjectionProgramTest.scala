...
class ProjectionProgramTest extends munit.FunSuite:
  ...
  test("ProjectionProgram computes divergence"):
    ...
    region.runUnsafe(
      ...
      onDone = layout => layout.divergence.read(divergenceOut),
    )
  ...
  test("ProjectionProgram solves pressure and removes divergence"):
    ...
    for i <- 0 until totalCells do divergenceBuffer.putFloat(0.3f) // Non-zero divergence
    ...
    divRegion.runUnsafe(
      ...
      onDone = layout => layout.divergence.read(divOut),
    )
    ...
    pressureRegion.runUnsafe(
      ...
      onDone = layout => layout.pressureCurrent.read(pressureBuffer),
    )
